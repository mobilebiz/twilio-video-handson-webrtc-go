(() => {
  ...
  toggle_camera.addEventListener("click", () => {
    ...
    root.style.setProperty(
      "--camera-icon",
      `url('./images/camera_${videoOn ? "on" : "off"}.png')`
    );
  });
  ...
  toggle_mic.addEventListener("click", () => {
    ...
    root.style.setProperty(
      "--mic-icon",
      `url('./images/mic_${audioOn ? "on" : "off"}.png')`
    );
  });
  ...
  const detectOwnSpeak = (analyser, timeDomain, frequency) => {
    setTimeout(() => {
      ...
      if (score > 10 && videoRoom && audioOn)
        dominantSpeaker({ sid: "my-video" });

      // スコアが10以下の場合は、自身のスタイルを削除
      if (score <= 10)
        document
          .getElementById("myStream")
          .classList.remove("dominant-speaker");
